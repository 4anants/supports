...
class EmailService {
    ...
    async sendEmail(to, subject, html) {
        ...
        const fromAddr = await prisma_1.default.settings.findUnique({ where: { key: 'smtp_from_address' } });
        const fromName = await prisma_1.default.settings.findUnique({ where: { key: 'smtp_from_name' } });
    ...
    formatTemplate(title, content, color = '#2563eb') {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    generateCardHtml(ticket, titleSub, backendUrl, frontendUrl, actionUrl, actionText, secondaryActionUrl, secondaryActionText) {
    ...
    async sendTestEmail(to) {
        const html = this.formatTemplate('SMTP Test Successful', `
    ...
    async sendStockUpdateNotification(updates, performedBy) {
        const frontendUrl = await this.getFrontendUrl();
        const teamEmails = await this.getTeamEmails();
        if (teamEmails.length === 0)
            return;
    ...
    async sendLowStockAlert(items) {
        const frontendUrl = await this.getFrontendUrl();
        const teamEmails = await this.getTeamEmails();
        if (teamEmails.length === 0 || items.length === 0)
            return;
